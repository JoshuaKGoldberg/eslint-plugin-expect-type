...
Thanks for your interest in contributing to `eslint-plugin-expect-type`! 💖
...
Please do [report an issue on the issue tracker](https://github.com/JoshuaKGoldberg/eslint-plugin-expect-type/issues/new/choose) if there's any bugfix, documentation improvement, or general enhancement you'd like to see in the repository! Please fully fill out all required fields in the most appropriate issue form.
...
With the exception of very small typos, all changes to this repository generally need to correspond to an [open issue marked as `accepting prs` on the issue tracker](https://github.com/JoshuaKGoldberg/eslint-plugin-expect-type/issues?q=is%3Aopen+is%3Aissue+label%3A%22accepting+prs%22).
If this is your first time contributing, consider searching for [unassigned issues that also have the `good first issue` label](https://github.com/JoshuaKGoldberg/eslint-plugin-expect-type/issues?q=is%3Aopen+is%3Aissue+label%3A%22accepting+prs%22+label%3A%22good+first+issue%22+no%3Aassignee).
...
Check [previously merged PRs](https://github.com/JoshuaKGoldberg/eslint-plugin-expect-type/pulls?q=is%3Apr+is%3Amerged+-label%3Adependencies+) for reference.
